...
volatile uint8_t capture_state = CAPTURE_IDLE;//表示当前捕获的为高/低电平
volatile uint16_t capture_val_up;//上升沿时计数器的值
volatile uint16_t capture_val_down;//下降沿时计数器的值
volatile uint8_t flag;//数据结束的标志位，结束时tag=1
