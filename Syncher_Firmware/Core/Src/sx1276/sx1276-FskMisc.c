...
uint8_t SX1276FskGetLnaGain(){
		uint8_t temp;
    SX1276Read( REG_LNA, &temp );
	  temp =temp>>5;
    return temp;
}
...
void SX1276FskSetRFFrequency( uint32_t freq )
{
    ...
    SX1276WriteBuffer( REG_FRFMSB, &SX1276->RegFrfMsb, 3 );
	  //Add high/low frequency mode switch
		SX1276->RegOpMode = SX1276FskGetOpMode();
		if(freq>779000000){	
			SX1276->RegOpMode = (SX1276->RegOpMode & RF_OPMODE_FREQMODE_ACCESS_MASK) | RF_OPMODE_FREQMODE_ACCESS_HF;
		}else{
			SX1276->RegOpMode = (SX1276->RegOpMode & RF_OPMODE_FREQMODE_ACCESS_MASK) | RF_OPMODE_FREQMODE_ACCESS_LF;
		}
